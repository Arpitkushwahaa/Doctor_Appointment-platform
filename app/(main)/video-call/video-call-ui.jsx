...
import { markUserJoinedCall, finalizeAppointmentStatus } from "@/actions/appointments";
...
export default function VideoCall({ sessionId, token }) {
  ...
  const initializeSession = () => {
    ...
    try {
      ...
      sessionRef.current.on("streamCreated", (event) => {
        console.log("New stream created - participant joined");
      ...
      // Handle when stream is destroyed (participant left)
      sessionRef.current.on("streamDestroyed", (event) => {
        console.log("Stream destroyed - participant left");
        setParticipantConnected(false);
        toast.info("Other participant left the call");
      });

      // Handle session events
      sessionRef.current.on("sessionConnected", async () => {
        setIsConnected(true);
        setIsLoading(false);

        // Mark that this user has joined the call
        if (appointmentId && !hasJoinedCall) {
          try {
  ...
  // End call
  const endCall = async () => {
    // If this is the doctor ending the call, finalize appointment status
    if (userRole === "DOCTOR" && appointmentId) {
      try {
        const formData = new FormData();
        formData.append("appointmentId", appointmentId);
        formData.append("patientJoined", participantConnected.toString());
        
        const result = await finalizeAppointmentStatus(formData);
        
        if (result.success) {
          if (result.status === "TIME_OVER") {
            toast.warning("Appointment marked as Time Over - Patient didn't join");
          } else {
