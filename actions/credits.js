...
const PLAN_CREDITS = {
  free_user: 2, // Free plan: 2 credits initially (one free appointment)
...
export async function checkAndAllocateCredits(user) {
  try {
    ...
    // For now, give all patients the free_user plan credits initially
    // This ensures everyone can book at least one appointment
    const currentPlan = "free_user";
    const creditsToAllocate = PLAN_CREDITS.free_user;
...
export async function deductCreditsForAppointment(userId, doctorId) {
  try {
    ...
    console.log(`Deducting credits: Patient ${userId} has ${user?.credits} credits, Doctor ${doctorId} has ${doctor?.credits} credits`);

    // Ensure user has sufficient credits
    if (user.credits < APPOINTMENT_CREDIT_COST) {
      console.error(`Insufficient credits: User ${userId} has ${user.credits} credits, needs ${APPOINTMENT_CREDIT_COST}`);
