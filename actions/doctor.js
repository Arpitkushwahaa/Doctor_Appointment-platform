...
/**
 * Gets doctor's availability settings
 */
export async function getDoctorAvailability() {
  ...
  try {
    // First verify the user is actually a doctor
    ...
    if (!user) {
      return { error: "User not found", slots: [] };
    }

    if (user.role !== "DOCTOR") {
      return { error: "Access denied. Only doctors can view this information.", slots: [] };
    }

    if (user.verificationStatus !== "VERIFIED") {
      return { error: "Doctor account not yet verified", slots: [] };
    }
    ...
    return { error: `Failed to fetch availability: ${error.message}`, slots: [] };
  }
}
...
export async function getDoctorAppointments() {
  ...
  try {
    ...
    if (user.role !== "DOCTOR") {
      throw new Error("Access denied. Only doctors can view this information.");
    }

    if (user.verificationStatus !== "VERIFIED") {
      throw new Error("Doctor account not yet verified");
    }

    // Now get the doctor's appointments
