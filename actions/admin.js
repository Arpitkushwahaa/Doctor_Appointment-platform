...
export async function verifyAdmin() {
  try {
    ...
    if (!userId) {
      console.log("DEBUG: verifyAdmin - No userId from auth");
      return false;
    }
    ...
    if (!user) {
      console.log("DEBUG: verifyAdmin - No user found in database");
      return false;
    }

    const isAdmin = user.role === "ADMIN";
    console.log("DEBUG: verifyAdmin - User role:", user.role, "Is admin:", isAdmin);
...
export async function getPendingDoctors() {
  try {
    ...
    if (!isAdmin) {
      return { error: "Unauthorized access", doctors: [] };
    }
    ...
    return { error: "Failed to fetch pending doctors", doctors: [] };
  }
}
...
export async function getVerifiedDoctors() {
  try {
    ...
    return { error: "Failed to fetch verified doctors", doctors: [] };
  }
}
...
export async function getPendingPayouts() {
  try {
    const isAdmin = await verifyAdmin();
    if (!isAdmin) {
      return { error: "Unauthorized access", payouts: [] };
    }
    ...
    return { error: "Failed to fetch pending payouts", payouts: [] };
  }
}
