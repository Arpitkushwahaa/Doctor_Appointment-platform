...
export function AppointmentCard({
  ...
  useEffect(() => {
    if (tokenData?.success) {
      // Redirect to video call page with token, session ID, appointment ID, and role
      router.push(
        `/video-call?sessionId=${tokenData.videoSessionId}&token=${tokenData.token}&appointmentId=${appointment.id}&role=${userRole}`
      );
    } else if (tokenData?.error) {
      setAction(null);
    }
  }, [tokenData, appointment.id, userRole, router]);
              ...
              <Badge
                ...
                className={
                  appointment.status === "COMPLETED"
                    ...
                    : appointment.status === "TIME_OVER"
                    ? "bg-orange-900/20 border-orange-900/30 text-orange-400"
                    : "bg-amber-900/20 border-amber-900/30 text-amber-400"
                }
              >
                {appointment.status === "TIME_OVER" ? "TIME OVER" : appointment.status}
              </Badge>
